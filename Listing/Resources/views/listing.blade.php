                    ...
                    @foreach ($actions as $action)
                        <div class="col-4 col-md-3">
                            <button type="button"
                                ...
                                data-name="{{ $action->getName() }}" data-url="{{ $action->getUrl() }}"
                                data-verb="{{ $action->getVerb() }}" data-method="{{ $action->getMethod() }}"
                                title="{{ strip_tags($action->getLabel()) }}"
                                data-confirmation="{{ $action->getConfirmationText() }}" data-bs-toggle="tooltip"
                                data-placement="top">
                                @if ($action->getIcon())
...
@if ($data && $columns)
    <div class="card-body px-0 pt-0 pb-2">
        <div class="table-resposive p-0">
            <table class="table table-striped table-hover align-items-center mb-0" id="listagemTable"
                data-redir="{{ url()->full() }}">
                ...
                <tbody>
                    ...
                    @forelse ($data->items() as $index => $item)
                        <tr
                            @if ($formToFieldId) data-search-field="{{ $formToFieldId }}"
                            data-search-value="{{ $item->$formFromFieldId }}" @endif>
                            ...
                            @foreach ($columns as $column)
                                <td
                                    style="word-wrap: break-word !important; overflow-wrap: break-word !important; white-space: normal !important;">
